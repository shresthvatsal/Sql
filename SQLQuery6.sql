...
--(select  EmployeeNumber, EmployeeFirstName, EmployeeLastName,
 ...
 --join tblEmployee as E on D.EmployeeNumber =E.EmployeeNumber ) as MyTable
--where TheRank <= 5
--order by EmployeeNumber
select * from
(select D.DepartmentID, EmployeeNumber, EmployeeFirstName, EmployeeLastName,
       rank() over(partition by D.DepartmentID order by E.EmployeeNumber) as TheRank
 from tblDepartment as D 
 join tblEmployee as E on D.DepartmentID = E.DepartmentID) as MyTable
where TheRank <= 5
order by DepartmentID, EmployeeNumber
...
CREATE TABLE tblWithRanking AS
...
FROM
    tblDepartment AS D
JOIN
    tblEmployee AS E ON D.DepartmentID = E.DepartmentID);
